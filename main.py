...
from scipy.constants import convert_temperature as temp
#instalacion de pyqt5: "pip install pyqt5 pyqt5-tools scipy"
...
globe_dictionary = {"Distance": {"Kilometers": 1000, "Meters": 1, "Centimeters": 0.01, "Millimeters": 0.001, "Miles": 1609.9, "Yards": 1093.61, "Feet": 3280.84, "Inches": 39370.1},
    ...
    "Temperature": {"Celsius": 1, "Fahrenheit": 1, "Kelvin": 1}, # bogus, luego modificar
    ...
    "Pressure": {"Pascals": 1, "Hectopascals": 100, "Kilopascals": 1000, "Megapascals": 1000000, "Bars": 100000, "PSI": 6894.76, "Atmospheres": 101325, "mmHg": 133.322, "Inches of mercury": 3386.39},
...
class MainWindow(Ui_UnitConversor):
    def __init__(self, parent=None):
        ...
        self.switch_2.pressed.connect(self.switch)
        self.input.textChanged.connect(self.convert) 
        self.from_dropbox.currentIndexChanged.connect(self.convert)
        self.to_dropbox.currentIndexChanged.connect(self.convert)
        # Modificar el QComboBox

    def update_selection(self, parent=None):

        global globe_dictionary

        self.from_dropbox.clear()
        self.to_dropbox.clear()
        if self.main_dropdown.currentText() != "--":
            ...
            self.from_dropbox.setCurrentIndex(0)
            self.to_dropbox.setCurrentIndex(1)
            self.convert()

    def switch(self): # Funcionalidad del boton
        from_index = self.from_dropbox.currentIndex()
        to_index = self.to_dropbox.currentIndex()
        self.from_dropbox.setCurrentIndex(to_index)
        self.to_dropbox.setCurrentIndex(from_index)
        self.from_dropbox.setText(self.to_dropbox.currentText())
        self.convert()
    
    def convert(self, parent=None):
        global globe_dictionary
        selection = self.main_dropdown.currentText() #tipo de conversion
        from_unit = self.from_dropbox.currentText() #unidad de conversion de entrada
        to_unit = self.to_dropbox.currentText() #unidad de conversion de salida (el)
        try:
            value = float(self.input.text())
        except:
            value = 0
        if selection != "Temperature":
            result = value * globe_dictionary[selection][from_unit] / globe_dictionary[selection][to_unit]
        else:
